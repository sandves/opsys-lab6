...
#define DEV_NAME "ptr_kernel_read"
...
static int dev_open (struct inode *inode, struct file *file) 
{
	...
	msgWritten = false;
	printk("ptr_kernel_read: skeleton device driver open\n");
	return 0;
}
...
{
	kfree( aBuff );
	printk("ptr_kernel_read: skeleton device driver closed\n");
	return 0;
}
...
static ssize_t dev_read (struct file *file, char *buf, size_t count, loff_t *ppos) 
{
	...
	int bufLen=0;
  	...
  	printk( "[ptr_kernel_read] This is the memory address I allocated = %s\n", pString );

  	bufLen = strlen( pString );
			
	if( copy_to_user( buf, pString, bufLen ) )
	{
		printk("ptr_kernel_read: copy_to_user failed\n" );
		return -EFAULT;
	}
	else {
		return bufLen; 
...
static void __exit dev_cleanup_module (void) 
{
	...
	printk("ptr_kernel_read: dev_cleanup_module Device Driver Removed\n");
}
